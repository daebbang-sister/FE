import Link from "next/link";

export default function Footer() {
  return (
    <footer className="text-neutral-0 flex min-h-[var(--size-footer-h)] w-full flex-col-reverse gap-10 bg-neutral-900 px-10 py-15 md:flex-row md:gap-0 lg:px-60">
      <section className="flex-1">
        <div className="font-poppins text-title2 mb-6">DAEBBANGSISTER</div>
        <ul className="text-body2 text-weight-regular flex flex-col gap-1.25 font-thin">
          <li>상호 : (주)대빵언니: 000</li>
          <li>주소 : 대구 달성군 현풍읍 테크노상업로 20 (테크노타워1) 107호</li>
          <li>사업자등록번호 : 000-00-00000</li>
          <li>통신판매업 : 000-00-00000</li>
          <li>연락처 (전화 또는 이메일)</li>
        </ul>
      </section>

      <section className="flex gap-15">
        <article>
          <p className="text-body1 mb-4">고객센터</p>
          <div className="mb-3 flex gap-2.5">
            <Link href="https://pf.kakao.com/_DHZxmG">
              <svg
                width="19"
                height="18"
                viewBox="0 0 19 18"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M9.5 0.5C14.5673 0.5 18.5 3.8266 18.5 7.72363C18.4999 11.6206 14.5672 14.9473 9.5 14.9473C8.92712 14.9473 8.36134 14.9015 7.81934 14.8193L7.62012 14.7891L7.45605 14.9053L4.16797 17.2295L4.95898 14.415L5.06934 14.0195L4.7041 13.834C2.14056 12.5331 0.500074 10.2551 0.5 7.72363C0.5 3.8266 4.43274 0.5 9.5 0.5Z"
                  fill="white"
                  stroke="white"
                />
                v
              </svg>
            </Link>
            <Link href="https://www.instagram.com/_daebbang_sister/">
              <svg
                width="18"
                height="18"
                viewBox="0 0 18 18"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <g clipPath="url(#clip0_127_8630)">
                  <path
                    d="M9 1.6207C11.4047 1.6207 11.6895 1.63125 12.6352 1.67344C13.5141 1.71211 13.9887 1.85977 14.3051 1.98281C14.7234 2.14453 15.0258 2.34141 15.3387 2.6543C15.6551 2.9707 15.8484 3.26953 16.0102 3.68789C16.1332 4.0043 16.2809 4.48242 16.3195 5.35781C16.3617 6.30703 16.3723 6.5918 16.3723 8.99297C16.3723 11.3977 16.3617 11.6824 16.3195 12.6281C16.2809 13.507 16.1332 13.9816 16.0102 14.298C15.8484 14.7164 15.6516 15.0188 15.3387 15.3316C15.0223 15.648 14.7234 15.8414 14.3051 16.0031C13.9887 16.1262 13.5105 16.2738 12.6352 16.3125C11.6859 16.3547 11.4012 16.3652 9 16.3652C6.59531 16.3652 6.31055 16.3547 5.36484 16.3125C4.48594 16.2738 4.01133 16.1262 3.69492 16.0031C3.27656 15.8414 2.97422 15.6445 2.66133 15.3316C2.34492 15.0152 2.15156 14.7164 1.98984 14.298C1.8668 13.9816 1.71914 13.5035 1.68047 12.6281C1.63828 11.6789 1.62773 11.3941 1.62773 8.99297C1.62773 6.58828 1.63828 6.30352 1.68047 5.35781C1.71914 4.47891 1.8668 4.0043 1.98984 3.68789C2.15156 3.26953 2.34844 2.96719 2.66133 2.6543C2.97773 2.33789 3.27656 2.14453 3.69492 1.98281C4.01133 1.85977 4.48945 1.71211 5.36484 1.67344C6.31055 1.63125 6.59531 1.6207 9 1.6207ZM9 0C6.55664 0 6.25078 0.0105469 5.29102 0.0527344C4.33477 0.0949219 3.67734 0.249609 3.10781 0.471094C2.51367 0.703125 2.01094 1.00898 1.51172 1.51172C1.00898 2.01094 0.703125 2.51367 0.471094 3.1043C0.249609 3.67734 0.0949219 4.33125 0.0527344 5.2875C0.0105469 6.25078 0 6.55664 0 9C0 11.4434 0.0105469 11.7492 0.0527344 12.709C0.0949219 13.6652 0.249609 14.3227 0.471094 14.8922C0.703125 15.4863 1.00898 15.9891 1.51172 16.4883C2.01094 16.9875 2.51367 17.2969 3.1043 17.5254C3.67734 17.7469 4.33125 17.9016 5.2875 17.9438C6.24727 17.9859 6.55312 17.9965 8.99648 17.9965C11.4398 17.9965 11.7457 17.9859 12.7055 17.9438C13.6617 17.9016 14.3191 17.7469 14.8887 17.5254C15.4793 17.2969 15.982 16.9875 16.4812 16.4883C16.9805 15.9891 17.2898 15.4863 17.5184 14.8957C17.7398 14.3227 17.8945 13.6688 17.9367 12.7125C17.9789 11.7527 17.9895 11.4469 17.9895 9.00352C17.9895 6.56016 17.9789 6.2543 17.9367 5.29453C17.8945 4.33828 17.7398 3.68086 17.5184 3.11133C17.2969 2.51367 16.991 2.01094 16.4883 1.51172C15.9891 1.0125 15.4863 0.703125 14.8957 0.474609C14.3227 0.253125 13.6687 0.0984375 12.7125 0.05625C11.7492 0.0105469 11.4434 0 9 0Z"
                    fill="white"
                  />
                  <path
                    d="M9 4.37695C6.44766 4.37695 4.37695 6.44766 4.37695 9C4.37695 11.5523 6.44766 13.623 9 13.623C11.5523 13.623 13.623 11.5523 13.623 9C13.623 6.44766 11.5523 4.37695 9 4.37695ZM9 11.9988C7.34414 11.9988 6.00117 10.6559 6.00117 9C6.00117 7.34414 7.34414 6.00117 9 6.00117C10.6559 6.00117 11.9988 7.34414 11.9988 9C11.9988 10.6559 10.6559 11.9988 9 11.9988Z"
                    fill="white"
                  />
                  <path
                    d="M14.8852 4.19404C14.8852 4.7917 14.4 5.27334 13.8059 5.27334C13.2082 5.27334 12.7266 4.78819 12.7266 4.19404C12.7266 3.59639 13.2117 3.11475 13.8059 3.11475C14.4 3.11475 14.8852 3.5999 14.8852 4.19404Z"
                    fill="white"
                  />
                </g>
                <defs>
                  <clipPath id="clip0_127_8630">
                    <rect width="18" height="18" fill="white" />
                  </clipPath>
                </defs>
              </svg>
            </Link>
          </div>
          <ul className="text-body2 flex flex-col gap-1.25 font-thin">
            <li>평일 오전10:00 ~ 오후6:00</li>
            <li>점심 오후12:00 ~ 오후1:00</li>
            <li>(공휴일 휴무)</li>
          </ul>
        </article>
        <article>
          <p className="text-body1 mb-4">서비스 정보</p>
          <ul className="text-body2 flex flex-col gap-1.25 font-thin">
            <li>
              <Link href="/terms" target="_blank" rel="noopener noreferrer">
                이용약관
              </Link>
            </li>
            <li>
              <Link href="/privacy" target="_blank" rel="noopener noreferrer">
                개인정보처리방침
              </Link>
            </li>
            <li>
              <Link href="/guide" target="_blank" rel="noopener noreferrer">
                이용안내
              </Link>
            </li>
          </ul>
        </article>
      </section>
    </footer>
  );
}
